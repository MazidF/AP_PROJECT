...
public class FollowRequestList extends Viewer {
    public User user;

    public FollowRequestList(Stage stage, Viewer lastViewer, User user) {
    ...
    public void initialization(Stage stage, double x, double y) {
        ...
        if (user.getFollowRequest().size() != 0) {
            List<Pane> panes = this.user.getFollowRequest().stream()
                    .map(userName -> new FollowRequest(stage, this, getServer().allUsers.get(userName)).getMadePane())
                    .collect(Collectors.toList());
            FollowerRequestList.getChildren().addAll(panes);
        } else {
            Label noRequest = new Label("no request yet.");
        ...
        Label requests = new Label("Requests");
        requests.setFont(Font.font("Tahoma", FontWeight.BOLD, 20));
        ...
        HBox requestsBox = new HBox();
        requestsBox.getChildren().add(requests);
        requestsBox.setAlignment(Pos.CENTER_RIGHT);

        HBox backBox = new HBox();
        backBox.getChildren().add(back);
        backBox.setAlignment(Pos.CENTER_LEFT);

        top.getChildren().addAll(backBox, requestsBox);
